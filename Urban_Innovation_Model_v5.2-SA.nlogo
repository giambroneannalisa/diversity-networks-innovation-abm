...
globals [
  ...
  mutation-prob
  imitation-prob
  ;; --- v5.2-SA: sensitivity-analysis globals (defaults reproduce v5.1 exactly) ---
...
to setup
  ...
  ;; v5.2-SA: sensitivity-analysis parameters survive clear-all too
  let saved-ccc coordination-cost-coefficient
  let saved-gcd kpf-gamma-cd
  let saved-bhc kpf-beta-hc
  let saved-jpb jump-prob-base
  let saved-rjm rescue-jump-mid
  let saved-rjx rescue-jump-max
  let saved-rth rescue-threshold
  let saved-rtl rescue-threshold-lo
  let saved-wsr ws-rewiring-prob
  let saved-mut mutation-prob
  let saved-ic  initial-cultures
  let saved-ksr knowledge-spillover-radius
  let saved-rft reinforcement-threshold
  ...
  set coordination-cost-coefficient saved-ccc
  set kpf-gamma-cd saved-gcd
  set kpf-beta-hc saved-bhc
  set jump-prob-base saved-jpb
  set rescue-jump-mid saved-rjm
  set rescue-jump-max saved-rjx
  set rescue-threshold saved-rth
  set rescue-threshold-lo saved-rtl
  set ws-rewiring-prob saved-wsr
  set mutation-prob saved-mut
  set initial-cultures saved-ic
  set knowledge-spillover-radius saved-ksr
  set reinforcement-threshold saved-rft
  ...
  ;; v5.2-SA defaults (a value of 0 means "not set": SA ranges start above 0)
  if (not is-number? coordination-cost-coefficient) or (coordination-cost-coefficient <= 0) [ set coordination-cost-coefficient 0.5 ]
  if (not is-number? kpf-gamma-cd) or (kpf-gamma-cd <= 0)           [ set kpf-gamma-cd 0.35 ]
  if (not is-number? kpf-beta-hc) or (kpf-beta-hc <= 0)             [ set kpf-beta-hc 0.30 ]
  if (not is-number? jump-prob-base) or (jump-prob-base <= 0)       [ set jump-prob-base 0.2 ]
  if (not is-number? rescue-jump-mid) or (rescue-jump-mid <= 0)     [ set rescue-jump-mid 0.5 ]
  if (not is-number? rescue-jump-max) or (rescue-jump-max <= 0)     [ set rescue-jump-max 0.8 ]
  if (not is-number? rescue-threshold) or (rescue-threshold <= 0)   [ set rescue-threshold 0.3 ]
  if (not is-number? rescue-threshold-lo) or (rescue-threshold-lo <= 0) [ set rescue-threshold-lo 0.15 ]
  if (not is-number? ws-rewiring-prob) or (ws-rewiring-prob <= 0)   [ set ws-rewiring-prob 0.10 ]
  if (not is-number? knowledge-spillover-radius) or (knowledge-spillover-radius <= 0) [ set knowledge-spillover-radius 3 ]
  set knowledge-spillover-radius round knowledge-spillover-radius
  if (not is-number? mutation-prob) or (mutation-prob <= 0) [ set mutation-prob 0.005 ]  ;; FIX 30b default
  ...
  if (not is-number? initial-cultures) or (initial-cultures < 2) [ set initial-cultures 8 ]  ;; FIX 25b default
  set initial-cultures round initial-cultures
  ...
  if not is-number? reinforcement-threshold [ set reinforcement-threshold 0 ]  ;; FIX 32 default (0 = simple contagion)
...
to update-firm-enhanced
  ...
  let alpha 0.25
  let beta  kpf-beta-hc
  let gamma kpf-gamma-cd
  let delta 0.10
  ...
  ;; --- 6. COORDINATION COST ---
  let coordination-cost (cultural-diversity ^ 2) * coordination-cost-coefficient
...
to setup-networks
  ...
  if count households > 1 [
    ...
    ask social-links [
      if random-float 1 < ws-rewiring-prob [
        let node1 end1
        die
        ask node1 [
...
to cultural-mutation-step
  let jump-prob jump-prob-base
  ;; If diversity is collapsing, increase probability of global jumps:
  if cultural-diversity-index < rescue-threshold [ set jump-prob rescue-jump-mid ]
  if cultural-diversity-index < rescue-threshold-lo [ set jump-prob rescue-jump-max ]
